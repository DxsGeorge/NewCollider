...
std::vector<Sphere> InitializeAtoms(int N, float R)
{
	...
	for (int i=0;i<N;i++){
		...
		sph.cm=Point(-50+rand()%100,-50+rand()%100,-50+rand()%100);
		sph.R=R;
...
std::vector<Molecule3> InitializeMol (int N, float R){
	...
	for (int i=0;i<N;i++){
		...
		m.cm=Point(-50+rand()%100,-50+rand()%100,-50+rand()%100);
		m.R=R;
		m.centersp=irr::core::vector3df(rand()%10,rand()%10,rand()%10);
		...
		temp1=irr::core::vector3df(-R*DIVSQRT2,-R*DIVSQRT2,0);
		temp2=irr::core::vector3df(R*DIVSQRT2,-R*DIVSQRT2,0);
		temp3=irr::core::vector3df(0,R*DIVSQRT2,0);
